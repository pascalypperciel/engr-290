...
void IMU_change_target_yaw(void);
bool IMU_is_turn_over(void);
float IMU_get_yaw(void);
float IMU_get_yaw_turning(void);
bool IMU_init(void);
float IMU_get(void);
...
float SENSORS_measure_distance(UltrasonicSensor US) {
	...
	// Checks when we receive a signal 
...
int SENSORS_opening_detected() {
	...
	// This logic makes sure there are enough good values in a row,
	// this was to combat the fact that we were having random spikes
	// in the distances read from the sensors.
	if (left >= RANGE_WALL) {
	...
	// Reset values if there wasn't enough good values in a row
	_last_was_big_right = false;
...
void SERVO_change_angle(float angle) {
	// Restricts the value because the servo only turns 180 degrees.
	if (angle < 0) angle = 0;
...
// All of the TWI functions (and defines, variables, and timer) were taken/inspired by this:
...
uint8_t TWI_read(uint8_t address, uint8_t* data, uint8_t length, uint8_t send_stop) {
	...
	while (_TWI_state != TWI_READY) {
		if ((GENERAL_micros() - start_micros) > 25000) {
			TWI_handle_timeout();
			return 1;
		}
	}
...
bool IMU_is_turn_over() {
	float current_yaw = IMU_get_yaw_turning();
...
float IMU_get_yaw() {
	...
	UART_send_string("\nYaw : \t");
	UART_send_string(buffer); // Prints the current yaw
	return yaw;
}

float IMU_get_yaw_turning() {
	char buffer[32];
	...
	UART_send_string("\nYee: \t");
	UART_send_string(buffer);	// Prints the current yaw and target yaw while turning
	
	return yaw;
}

//	The following of the IMU functions were inspired by:
//	https://github.com/maarten-pennings/MPU6050
bool IMU_init() {
	if (IMU_absent() || IMU_calibrate() || IMU_wake() || IMU_set_rate_divider() || IMU_set_DLPF_bandwidth()) {
		UART_send_string("\nError during IMU_init()");
		return false;
	}

	IMU.DIRECTIONS.TIME = GENERAL_millis();
	IMU.DIRECTIONS.ANGLE_Z = 0;
...
float IMU_read_gyroscope_yaw() {
	int16_t gyro_raw_z;
	IMU_read_3x16(0x43, NULL, NULL, (uint16_t*)&gyro_raw_z);
...
void IMU_update_yaw() {
	...
	IMU.DIRECTIONS.YAW = IMU.DIRECTIONS.ANGLE_Z;
}
...
//Replacement for Arduino's millis()
...
//Replacement for Arduino's micros()
unsigned long GENERAL_micros() {
	unsigned long m;
...
void GENERAL_components_setup() {
	// Initial Servo angle and fan speeds
...
void GENERAL_go_forward() {
	// This code uses IMU data to make sure the hovercraft goes straight
	...
	char buffer[32];
	snprintf(buffer, sizeof(buffer), "\tServo = %d\r\t", (int)imu_error); // This prints the current angle of the servo
	UART_send_string(buffer);
...
void GENERAL_make_turn(float opening_angle) {
	// Lowering steering fan's speed, turns the servo and toggles the target yaw
	FAN_set_spin(FAN_STEER, FAN_SPEED_LESS_LESS);
	SERVO_change_angle(opening_angle);
	IMU_change_target_yaw();
	
	// Keep going until we reach the target yaw (with error margin)
	while (!IMU_is_turn_over());
	
	// Increase steering fan's speed and turns the servo so it goes forward
	FAN_set_spin(FAN_STEER, FAN_SPEED_LESS);
...
int main(void) {
	...
	int can_turn_counter = WAIT_AFTER_TURN;
	_opening_in_a_row_right = 0;
	_opening_in_a_row_left = 0;
	_last_was_big_right = false;
	_last_was_big_left = false;
	...
	while (1) {
		// This value is there so we ignore openings for a certain amount of time
		if (can_turn_counter > 0) {
			can_turn_counter--;
		}
		GENERAL_go_forward();
		
		int opening_angle = SENSORS_opening_detected();
		if (opening_angle != -1 && can_turn_counter == 0) {
			GENERAL_make_turn(opening_angle);
			can_turn_counter = WAIT_AFTER_TURN;
		}
	}
}
